...
class DenseBlock(nn.Module):
    def __init__(self, nb_layers, in_planes, growth_rate, 
        ...
        self.layer = self._make_layer(block, in_planes, growth_rate,
                                        nb_layers, dropRate)
    def _make_layer(self, block, in_planes, growth_rate,
                    nb_layers, dropRate):
        layers = []
...
class DenseNet3(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12,
                 reduction=0.5, bottleneck=True, dropRate=0.0,
                    use_sdr=False, beta=0.1, zeta=0.8, zeta_drop=1):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                ## uncomment for uniform distribution
                #m.weight.data.uniform_(-math.sqrt(2. / n), math.sqrt(2. / n))
